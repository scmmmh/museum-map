    ...
    <div class="flex flex-col md:grid md:grid-cols-12 gap-8 p-4">
        {#if $config && $config.attributes.intro && (!$localPreferences.lobby  || !$localPreferences.lobby.hideWelcome)}
            <section class="col-span-12 my-5">
                <div class="relative max-w-4xl mx-auto border border-neutral-500 shadow-xl px-2 py-2">
                    <button on:click={() => { localPreferences.setPreference('lobby.hideWelcome', true); }} class="block absolute right-0 top-0 lg:transform lg:translate-x-1/2 lg:-translate-y-1/2 rounded-full text-lg bg-neutral-600 border border-neutral-500 z-10 shadow" aria-label="Close">
                        ...
                        <li>
                            ...
                            <ul class="flex flex-row flex-wrap gap-2">
                                {#each collection.floors as floor}
                                    <li class="flex-none"><Link to={'/floor/' + floor.id} class="inline-block bg-neutral-600 px-4 lg:px-3 py-3 lg:py-1 rounded-lg lg:underline-offset-2 lg:hover:bg-blue-800 lg:focus:bg-blue-800">⇒ {floor.attributes.label}</Link></li>
                                {/each}
                            </ul>
                        </li>
                    {/each}
        ...
        <section class="col-span-12 mt-4">
            <div class="flex flex-row">
                ...
                <button on:click={() => { fetchRandomItemsSelection(); }} class="flex-none px-2 py-2" aria-label="Update the selection of items from the collection" title="Update the selection of items from the collection">
